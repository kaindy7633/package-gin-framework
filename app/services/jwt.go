...
func (jwtService *jwtService) CreateToken(AppGuardName string, user JwtUser) (tokenData TokenOutPut, err error, token *jwt.Token) {
...
func (jwtService *jwtService) IsInBlacklist(tokenStr string) bool {
	...
	// JwtBalcklistGracePeriod 为黑名单宽限时间，避免并发请求失效
